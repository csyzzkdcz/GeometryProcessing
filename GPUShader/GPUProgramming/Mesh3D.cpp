...
bool Mesh3D::LoadFromOBJFileWithNormal(const char* fins)
{
	FILE *pfile = fopen(fins, "r");
	...
	try
	{
		...
		while(fgets(pLine, 512, pfile))
		{
			if (pLine[0]=='v' && pLine[1]=='n' && pLine[2]==' ')
			{
				Vec3f vn;
				tok = strtok(pLine," ");
				for (int i=0; i<3; i++)
				{
					...
					strcpy(temp, tok);
					temp[strcspn(temp," ")] = 0;
					vn[i] = (float)atof(temp);
				}
				normalVec.push_back(vn);
			}
		}
			...
			if (texCoordsTemp.empty())
			{
				...
				while (fscanf(pfile, "%s", pLine) != EOF)
				{
					...
					if (pLine[0] == 'f')
					{
						int v, n;
						fscanf(pfile, "%s", pLine);
						if (sscanf(pLine, "%d//%d", &v, &n) == 2)
						{
							std::map<int, int> v2tex;
							v2tex[v - 1] = n - 1;

							fscanf(pfile, "%s", pLine);
							sscanf(pLine, "%d//%d", &v, &n);
							v2tex[v - 1] = n - 1;
							...
							HE_edge* edgeTemp = pfaces_list_->at(faceIndex)->pedge_;
							edgeTemp->pvert_->normal_ = normalVec.at(v2tex[edgeTemp->pvert_->id_]);	
							edgeTemp = edgeTemp->pnext_;
							edgeTemp->pvert_->normal_ = normalVec.at(v2tex[edgeTemp->pvert_->id_]);	
							edgeTemp = edgeTemp->pnext_;
							edgeTemp->pvert_->normal_ = normalVec.at(v2tex[edgeTemp->pvert_->id_]);	
			...
			else
			{
				...
				while (fscanf(pfile, "%s", pLine) != EOF)
				{
					...
					if (pLine[0] == 'f')
					{
						int v, t, n;
						fscanf(pfile, "%s", pLine);
						if (sscanf(pLine, "%d/%d/%d", &v, &t, &n) == 3)
						{
							std::map<int, int> v2tex;
							v2tex[v - 1] = n - 1;

							fscanf(pfile, "%s", pLine);
							sscanf(pLine, "%d/%d/%d", &v, &t, &n);
							v2tex[v - 1] = n - 1;
...
void Mesh3D::WriteToOBJFileWithNormal(const char* fouts)
{
	std::ofstream fout(fouts);
	...
	for (;fiter!=pfaces_list_->end(); fiter++) 
	{
		...
		do {
			fout<<" "<<edge->ppair_->pvert_->id_+1<<"//"<<edge->ppair_->pvert_->id_+1;
			edge = edge->pnext_;
...
void Mesh3D::ComputePerFaceNormal(HE_face* hf)
{
	...
	hf->normal_ = nc.crossProduct(pc);	// cross prodoct
