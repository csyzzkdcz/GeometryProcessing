...
void RenderingWidget::SetMethodAsLocal()
{
	methods_ = kLocal;
	Editor_ = new MiniSurfaceLocal();
	switch (BC_)
	{
	case kAverage:
		Editor_->BC_Editor_ = new BC_Average();
		break;
	case kMeanValue:
		Editor_->BC_Editor_ = new BC_MeanValue();
		break;
	case kDiscretHarmonic:
		Editor_->BC_Editor_ = new BC_DiscretHarmonic();
		break;
	case kNone:
		break;
	default:
	...
	for (int i = 0; i < REFINE_TIME; i++)
	{
		DoMiniSurface();
	}
	Editor_->tar_mesh_ = NULL;
	ptr_mesh_->UpdateMesh();
	updateGL();
	clock_t end_ = clock();
	double time_ = double(end_ - start_);
	cout << "Running time" << time_/1000 << endl;
}

void RenderingWidget::SetMethodAsGlobal()
{
	methods_ = kGlobal;
	Editor_ = new MiniSurfaceGlobal();
	switch (BC_)
	{
	case kAverage:
		Editor_->BC_Editor_ = new BC_Average();
...
void RenderingWidget::CheckAve(bool bv)
{
	if (bv)
		BC_ = kAverage;
}

void RenderingWidget::CheckMV(bool bv)
{
	if (bv)
		BC_ = kMeanValue;
}

void RenderingWidget::CheckDH(bool bv)
{
	if (bv)
		BC_ = kDiscretHarmonic;
}
...
void RenderingWidget::CheckEllipse(bool bv)
{
	if (bv)
		type_ = kEllipse;
}
...
void RenderingWidget::DoMiniSurface()
{
	Editor_ ->Init(ptr_mesh_);
	Editor_->MiniSurface_Method();
	ptr_mesh_ = Editor_->tar_mesh_;
}

void RenderingWidget::GenerateMyMesh()
{
	switch (type_)
	{
	case kCircle:
		boundary_ = new Boundary_Ellipse(1, 1);
		break;
	case kEllipse:
		boundary_ = new Boundary_Ellipse(2, 1);
		break;
	case kRect:
		boundary_ = new Boundary_Rect(4, 2);
		break;
	case kNoneType:
		boundary_ = NULL;
		break;
	...
	if (boundary_)
	{
		boundary_->Curves(rx_array_, ry_array_);
		GenerateMesh *my_genterator_ = new GenerateMesh(rx_array_, ry_array_);
		my_genterator_->DoGenerating();
		ptr_mesh_ = my_genterator_->my_mesh_;
		updateGL();
	}
}