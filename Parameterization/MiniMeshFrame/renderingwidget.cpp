...
has_lighting_(false), is_draw_point_(true), is_draw_edge_(false), is_draw_face_(false), is_draw_texture_(false)
{
	...
	// Initialization
	B_Constraint_ = kNoneConstraint;
	B_Type_ = kNoneType;
	P_FB_Method_ = kNoneFBMethod;
	P_UFB_Method_ = kNoneUFBMethod;
	P_Editor_ = NULL;
}
...
void RenderingWidget::ReadMesh()
{
	...
	prev_vertices_.clear();
	for (int i = 0; i < ptr_mesh_->num_of_vertex_list(); i++)
	{
		prev_vertices_.push_back(ptr_mesh_->get_vertex(i)->position_);
	}
	updateGL();
}
...
void RenderingWidget::DrawTexture(bool bv)
{
	...
	for (size_t i = 0; i != faces.size(); ++i)
	{
		...
		do
		{
	/*		pedge->pvert_->texCoord_[0] = ptr_mesh_->get_vertex(pedge->pvert_->id())->position_[0]/2.0;
			pedge->pvert_->texCoord_[1] = ptr_mesh_->get_vertex(pedge->pvert_->id())->position_[1]/2.0;
		//	pedge->pvert_->texCoord_[2] = ptr_mesh_->get_vertex(pedge->pvert_->id())->position_[2]/2.0;*/
...
void RenderingWidget::CheckFB(bool bv)
{
	if (bv)
		B_Constraint_ = kFixed;
}

void RenderingWidget::CheckUFB(bool bv)
{
	if (bv)
		B_Constraint_ = kUnfixed;
}

void RenderingWidget::CheckCircle(bool bv)
{
	if (bv)
		B_Type_ = kCircle;
}

void RenderingWidget::CheckRect(bool bv)
{
	if (bv)
		B_Type_ = kRect;
}
...
{
	if (bv)
		P_FB_Method_ = kUniform;
}

void RenderingWidget::CheckWLS(bool bv)
{
	if (bv)
		P_FB_Method_ = kWLS;
}

void RenderingWidget::CheckSP(bool bv)
{
	if (bv)
		P_FB_Method_ = kSP;
}

void RenderingWidget::CheckARAP(bool bv)
{
	if (bv)
		P_UFB_Method_ = kARAP;
}

void RenderingWidget::DoParameterization()
{
	if (ptr_mesh_->num_of_vertex_list() == 0)
		return;
	switch (B_Constraint_)
	{
	case kNoneConstraint:
		break;
	case kUnfixed:
		switch (P_UFB_Method_)
		{
		case kARAP:
			P_Editor_ = new Parameterization_ARAP(ptr_mesh_);
			P_Editor_->Do_Parameterizing(*(ptr_mesh_->get_vertex_list()));
			break;
		default:
			break;
		}
	...
	case kFixed:
		...
		case kCircle:
			...
			{
			case kUniform:
				P_Editor_ = new Parameterization_FC_Uniform();
				P_Editor_->Do_Parameterizing(*(ptr_mesh_->get_vertex_list()));
				break;
			case kWLS:
				P_Editor_ = new Parameterization_FC_WLS();
				P_Editor_->Do_Parameterizing(*(ptr_mesh_->get_vertex_list()));
				break;
			case kSP:
				P_Editor_ = new Parameterization_FC_SP();
				P_Editor_->Do_Parameterizing(*(ptr_mesh_->get_vertex_list()));
				break;
			case kNoneFBMethod:
				break;
		...
		case kRect:
			...
			{
				case kUniform:
					P_Editor_ = new Parameterization_FS_Uniform();
					P_Editor_->Do_Parameterizing(*(ptr_mesh_->get_vertex_list()));
					break;
				case kWLS:
					P_Editor_ = new Parameterization_FS_WLS();
					P_Editor_->Do_Parameterizing(*(ptr_mesh_->get_vertex_list()));
					break;
				case kSP:
					P_Editor_ = new Parameterization_FS_SP();
					P_Editor_->Do_Parameterizing(*(ptr_mesh_->get_vertex_list()));
					break;
				case kNoneFBMethod:
					break;
	...
	if (B_Constraint_ != kNoneConstraint)
	{
		// Update Texture
		for (int i = 0; i < ptr_mesh_->num_of_vertex_list(); i++)
		{
			ptr_mesh_->get_vertex(i)->texCoord_[0] = ptr_mesh_->get_vertex(i)->position_[0];
			ptr_mesh_->get_vertex(i)->texCoord_[1] = ptr_mesh_->get_vertex(i)->position_[1];
		}
...
void RenderingWidget::RestoreMesh()
{
	...
	for (int i = 0; i < ptr_mesh_->num_of_vertex_list(); i++)
	{
		ptr_mesh_->get_vertex(i)->position_ = prev_vertices_[i];
	}
	ptr_mesh_->UpdateNormal();
	updateGL();
}